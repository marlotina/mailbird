...
namespace Reading.Mails.NUnitTest.Domain.Model
{
    ...
    public class EmailBodyPetitionTests
    {
        ...
        public void IsValidItem(string serverType, string server, int port, string encryption, string username, string password, int emailId)
        {
        ...
        [TestCase("IMAP", "imap.smtp", 143, "UNENCRYPTED", "test@gmail.com", "asdsa", 12350)]
        public void NotValidItem(string serverType, string server, int port, string encryption, string username, string password, int emailId)
        {
            var emailProviderConfig = new EmailBodyPetition(serverType, server, port, encryption, username, password, emailId);
            Assert.IsTrue(!emailProviderConfig.IsValidData());
        }
